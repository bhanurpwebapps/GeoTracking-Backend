...
async function insertAttendance(gatewayMacId, areaId, areaName, client, student) {
  ...
  try {
    ...
    const lastAttendanceStatus = existingAttendance[0].attendanceStatus;
    ...
    if (
      schoolStartTime.diff(now, 'milliseconds') > classroomTimeoutMs && now.isBefore(schoolStartBuffer) &&
      !hasStatus('Present') &&
      areaId === student.classRoom.toString()
    ) {
      await markAttendance(student, studentId, clientId, 'Present', areaName, areaId, deviceId, 'Authorized');
      console.log('Student marked as Present (school start > classroom timeout).');
      sendUnauthorizedZoneAlert(student, areaName);
      return;
    }
    ...
    if (status === 'UnAuthorized') {
      if (now.diff(getLastDetectionTime(existingAttendance) || now.toDate(), 'milliseconds') > unauthorizedZoneTimeout * 60000 && areaName !== 'Exit') {
        await markAttendance(student, studentId, clientId, existingAttendance?.[0]?.attendanceStatus, areaName, areaId, deviceId, status);
        sendUnauthorizedZoneAlert(student, areaName);
      }
    }
    else {
